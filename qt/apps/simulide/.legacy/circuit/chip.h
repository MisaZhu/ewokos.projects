 ...
 *
 * The fallback preserves one invariant a subclass is entitled to rely on,
 * because initChip() has it too: the index of a pin is its datasheet number less
 * one.  initChip() gets that from indexing by a running counter over the <pin>
 * elements in document order, which for a package file written in datasheet order
 * is the same thing; the fallback gets it by laying both sides out in increasing
 * pin number.  So "pin 14 is RESET" is index 13 either way, and a device that
 * hardcodes its role indices does not have to know which path ran.
 */
