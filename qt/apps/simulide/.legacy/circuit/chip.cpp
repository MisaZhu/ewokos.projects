...
void Chip::buildDipFallback( int pins )
{
    ...
    // numbering every DIP datasheet uses and the one a user counts by.
    //
    // Both loops run in increasing pin number, and that is load-bearing rather
    // than a tidiness: addChipPin() appends, so the index a pin gets is the order
    // it was added in, and running them this way makes
    //
    //        index == datasheet pin number - 1
    //
    // hold for every pin of the package.  Upstream's hardcoded DIP components
    // rely on exactly that - its Lm555 puts Gnd at m_pin[0], CV at m_pin[4] and
    // Vcc at m_pin[7] - and Chip::initChip() gets the same property from a
    // .package file whose <pin> elements are in datasheet order, because it too
    // indexes by a running counter over document order.  So a subclass that
    // writes its role indices as pin numbers is right whether the file was found
    // or this fallback ran, which is the only reason the fallback is safe to fall
    // back to.
    //
    // The positions are unaffected: the right side still runs pin `pins` at the
    // top down to pin leftCount+1 at the bottom.  What changed is which index
    ...
    for( int i = leftCount+1; i <= pins; i++ )
    {
        const QString num = QString::number( i );
